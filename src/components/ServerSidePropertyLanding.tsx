  ...
  private trimCache() {
    if (this.preloadedUrls.size > this.maxCacheSize) {
      const toDelete = Array.from(this.preloadedUrls).slice(
        0,
        this.preloadedUrls.size - this.maxCacheSize
      );
      toDelete.forEach((url) => this.preloadedUrls.delete(url));
    }
    if (this.failedUrls.size > this.maxCacheSize / 2) {
      const toDelete = Array.from(this.failedUrls).slice(
        0,
        this.failedUrls.size - this.maxCacheSize / 2
      );
...
const useImagePreloader = (
  ...
  const preloadOnHover = useCallback(() => {
    if (isHovered || images.length <= 1) return;
    setIsHovered(true);
    const maxAhead = isCoarseRef.current ? 2 : 4;
    const toPreloadIdx = Array.from(
      { length: maxAhead },
      (_, k) => k + 1
    ).filter((i) => i < images.length);

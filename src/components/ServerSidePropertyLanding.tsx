...
====================================== */
// Szűkített, valós layouthoz illő deviceSizes – kevesebb transform = olcsóbb
const DEVICE_SIZES = [360, 400, 414, 640, 828];

// (Opcionális alsó korlát a kártyaszélességhez)
...
function pickWidthDynamic() {
  ...
  const dpr = Math.max(1, Math.min(3, Math.round((typeof window !== "undefined" ? window.devicePixelRatio : 1) || 1)));
...
const useImagePreloader = (
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && typeof window.matchMedia !== "undefined") {
      try { isCoarseRef.current = window.matchMedia("(pointer: coarse)").matches; } catch {}
    }
  }, []);
...
const PropertyCard: React.FC<{ property: Property; onClick: () => void }> = ({ property, onClick }) => {
  ...
  useEffect(() => {
    if (visible) preloadOnDemand(); // láthatóvá váláskor egyszer tűzel (mobilon is)
  ...
  const onFirstTouchStart = useCallback(() => {
    ...
    preloadOnDemand(); // első érintésre
  }, [preloadOnDemand]);
    ...
    async (index: number) => {
      ...
      try {
        ...
        if (myToken === switchTokenRef.current) {
          ...
          // blur/átmenet vége majd onLoadingComplete-ben
    ...
    >
      <div className="relative h-64 overflow-hidden bg-gray-100">
        <Image
          ...
          fill
          /* Rács-igazított, költségbarát sizes:
             1 col ≤640px: 100vw
             2 col ≤1024px: 50vw
             3 col ≤1280px: 33vw
             4 col >1280px: 25vw
             Ha 400px felső korlátot szeretnél, az utolsó ág lehet "400px".
          */
          ...
          onLoadingComplete={() => {
            ...
            setIsSwitching(false); // blur/átmenet vége csak a ténylegesen betöltött példány után
          }}
        />
    ...
    async (newFilters: PropertySearchParams) => {
      startTransition(async () => {
        try {
          ...
          // opcionális: első képek "melegítése" NEXT-opt URL-re
